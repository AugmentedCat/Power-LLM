...
class Generator(object):
    ...
    def initialize(self, from_model: Optional[str] = None):
        ...
        if from_model:
            ckpt = torch.load(from_model, map_location='cpu', weights_only=False)
            self.model.load_state_dict(ckpt['model'])

        # Move the model to GPU device.
        # Note: Not using half precision due to large activations in power layers
