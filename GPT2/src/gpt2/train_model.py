...
class GPT2TrainingSpec(TrainingSpec):
    def __init__(self, train_corpus: str, eval_corpus: str, vocab_path: str,
                 ...
                 total_steps: int, use_grad_ckpt: bool, use_relu: bool,
                 use_post_power_norm: bool):
    ...
    def initialize(self):
        self.vocab = Vocab(vocab_path=self.vocab_path)  # vocab_path can be None now
        print(f"Using GPT-2 tokenizer (vocab size: {len(self.vocab)})")
    ...
    def construct_model(self) -> nn.Module:
        return Transformer(layers=self.layers, pad_idx=self.vocab.pad_idx,
                           ...
                           dropout=self.dropout, bidirectional=False, use_relu=self.use_relu,
                           use_post_power_norm=self.use_post_power_norm)
...
def train_gpt2_model(args: argparse.Namespace):
    spec = GPT2TrainingSpec(
        ...
        use_relu=args.use_relu, use_post_power_norm=args.use_post_power_norm)
    config = TrainConfig(
...
def add_subparser(subparsers: argparse._SubParsersAction):
    ...
    group.add_argument('--vocab_path', default=None,
                       help='vocabulary file path (optional - uses GPT-2 pretrained if not provided)')
    ...
    group.add_argument('--use_relu', action='store_true', default=True,
                       help='use ReLU after power transformation (default: True)')
    group.add_argument('--no_relu', dest='use_relu', action='store_false',
                       help='disable ReLU after power transformation (use pure power scaling)')
    group.add_argument('--use_post_power_norm', action='store_true', default=False,
                       help='normalize activations after power transformation (prevents explosion)')
    group.add_argument('--no_post_power_norm', dest='use_post_power_norm', action='store_false',
                       help='disable post-power normalization (default)')
    ...
    group.add_argument('--use_amp', action='store_true',
                       help='use automatic mixed-precision (BF16) in training for 9x speedup and 43%% memory savings')
    group.add_argument('--use_grad_ckpt', action='store_true',
                       help='use gradient checkpointing in transformer layers')
    group.add_argument('--grad_clip_norm', default=0.0, type=float,
                       help='gradient clipping max norm (DISABLED by default)')
